...
export function NewNoteCard({onNoteCreated}: NewNoteCardProps) {
    ...
    function handleIsRecording()
    {
        ...
        if(!isSpeechRecognitionApiAvailable){
            alert("Infelizmente seu navegador não suporta o gravador")
            return
        }
        setIsRecording(true)
        setShouldShowOnboarding(false)
            ...
            <form  className='flex-1 flex flex-col'>
                <div className='flex flex-1 flex-col gap-3 p-5'>
                    ...
                    {shouldShowOnboarding ? (
                        <p className="text-sm leading-6 text-slate-400">
                            Comece <button type='button' onClick={handleIsRecording} className='font-medium text-lime-400 hover:underline'>gravando uma nota</button> em áudio ou se preferir <button type='button' onClick={handleStartEditor} className='font-medium text-lime-400 hover:underline'>utilize apenas texto</button>.
                        </p>
                    ) : (
                        ...
                        {isRecording ? (
                            ...
                            Gravando nota (Clique p/ interromper.)
                            </button>
                        ):(
                            <button 
                        type='button'
